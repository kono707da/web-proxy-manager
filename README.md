...
- **代理状态可视化**：顶部实时状态条显示内核运行/停止、代理模式、当前生效节点；订阅页已启用标记；节点页显示来源订阅与测速。
- **设备管理**：按来源 IP 为每台设备分配固定代理节点，所有设备共用 7890 端口，无需认证。在代理节点页面通过设备下拉框选择设备、订阅下拉框筛选节点，点击节点即为该设备分配代理节点。支持自动发现未分配客户端。
